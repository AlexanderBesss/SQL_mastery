...
-- Create Schema and drop it
create schema if not exists playground;
create table if not exists playground.users(
	user_id serial constraint pk_index_123 primary key, -- constraint pk_index_123 is name of the index
	first_name varchar(100) not null,
	last_name varchar(100) not null,
	email varchar(100) not null unique,
...
create table if not exists playground.notes(
	...
	foreign key (user_id) references playground.users(user_id)
);
